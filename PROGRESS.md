...
| **V12 — prove the earn base on a tax-INCLUSIVE shop** | **GO-LIVE BLOCKER, added 3 Sep 2026. The development-store route is closed — do not reopen it.** The earn base subtracts both the discount allocation and tax when an order is tax-inclusive, and **only the tax-exclusive branch is proved**. The dev store cannot prove the other: its merchant address is locked to the United States, and a non-UK merchant is not required to collect UK VAT on orders over £135, so every UK checkout above that re-prices to base at the payment step with no tax line. Reproduced twice on 3 Sep 2026. OSC sells VAT-inclusive, so the unproved branch is the one that ships. **Closable only by reconciling one real discounted order on the live store before launch** — compare `gross − allocations − tax` against the VAT-exclusive value the member actually spent. See V12 in `DECISIONS.md` |
| **V14 — reported spend after a correction** | **Sprint 5 gate, added 3 Sep 2026.** A compensating `earn_reversal` carries no `qualifying_value_pence`, so `ProgrammeSummary` and `MemberPresenter` overstate spend by the corrected amount — account 10 sums to £600 against a real £550. Points, balances, maturity and segmentation are all unaffected. Held rather than fixed: the candidate fix changes what the column means, and the Sprint 5 reporting design has to agree with that first. See V14 in `DECISIONS.md` |
| **V16 — a till user with no role cannot use the tile** | **GO-LIVE BLOCKER, added 3 Sep 2026.** Only the first staff member on a shop is bootstrapped; everyone after gets `403 no_role_assigned` and the tile reads as broken. Proved on the first real POS token — the deep-link preview surface authenticates as the installing account, POS proper does not. Either OSC assigns a role to every till user as a documented onboarding step, or `EnsureStaffRole` grants an implicit `viewer` floor to a verified POS token; the decision is parked next to C9. See V16 in `DECISIONS.md` |
